...
def get_image_optimal_window_info_drjc(image, com, window_dim,
                                  step=1, ):
    """ Return centre of image only. Focuses less on fibroglandular cone then Wu/NYU et al 2019
    Includes axilla (which is likely predictive in some cases) and tissue close to chest wall
    (where tumours are deeper, harder to palpate and important for screening programs to detect) """    
    return {
        "window_dim_y": window_dim[0],
        "window_dim_x": window_dim[1],
        "best_center_y": image.shape[0]/2,
        "best_center_x": image.shape[1]/2,
        "fraction": 0}
...
def get_candidate_center_topleft_bottomright(com, image_dim, window_dim, step):
    """
    bottom right (br) returned is exclusive
    """
    half_dim = window_dim // 2
    rem_window_dim = window_dim - half_dim #remaining_window_dimension

    # Find extremes of possible centers, based on window size
    center_tl = half_dim
    center_br = image_dim - rem_window_dim
    
    x = center_tl[1], center_br[1]
    y = center_tl[0], center_br[0]
    #
...
def get_rightmost_pixel_constraint(rightmost_x):
    ...
    def _f(tl, br, image, window_dim, rightmost_x_=rightmost_x):
        ...
        tl = tl.copy()
        br = br.copy()
#        print('calc_optimal_centers.get_rm_pix.rightmost_x:')
#        print(rightmost_x_)
#        print(type(rightmost_x_))
        new_x = rightmost_x_ - half_dim_x
        tl[1] = new_x - 1
        br[1] = new_x
        return tl, br #top left, bottom right
