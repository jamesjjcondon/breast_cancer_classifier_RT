...
def unpack_exam_into_images(exam_list, cropped=False, centred=False):
    """
    Turn exam_list into image_list for parallel functions which process each image separately.
    """
    assert isinstance(exam_list, list)
    
    image_list = []
    for i, exam in enumerate(exam_list):
        for view in VIEWS.LIST:
            assert isinstance(exam[view], list)
#            print(view)
...
#                print(type(image))
                ...
                if centred:
                    image_dict["meta_pre_pad"] = exam['best_center'][view][j]
#                    image_dict["fraction"] = exam["fraction"][view][j]
