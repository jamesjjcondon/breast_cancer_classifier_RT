...
def main(hparams):
    ...
    iteration = 'Redo_12NOV_V1_restart1'
    #iteration = '24May_2GPU_V6'
    print('\n\tIteration/logs name:', iteration)
    
    callback_dir = os.path.join(DATADIR,'dev_test_models/1-channel/ckpts_' + iteration +'/')
    ...
    logger = TestTubeLogger(
            save_dir=os.path.join(DATADIR, 'dev_test_logs/tt_logs/1-channel'),
            name='NYURT_'+iteration,
            description="NYU1_rand_init",
            debug=False,
            version=None)
...
#        overfit_pct=hparams.overfit_pct, 
    ...
    if hparams.rand_init:
        ...
        model.load_state_dict(
                torch.load(
                        os.path.join(REPODIR, 'models/sample_image_model.p')
                        )["model"]
                )
...
if __name__ == '__main__':
    ...
    parser.add_argument('--check_val_n', type=int, default=5)
    parser.add_argument('--device_type', type=str, default='gpu')
    parser.add_argument('--gpus', default=-1)
    parser.add_argument('--grad_cum', default=6)
    parser.add_argument('--lr', type=float, default=1e-08) # if using cyclic schedule, == bottom of cycle/min
    ...
    parser.add_argument('--restore', type=str, #default=None)
                                default='/data/james/NYU_retrain/dev_test_models/1-channel/ckpts_Redo_12NOV_V1/Redo_12NOV_V1_epo=99_tloss=1.386_avloss=2.431.ckpt')
    ...
    parser.add_argument('--use_heatmaps', action='store_true', default=False)
    ...
    parser.add_argument('--weight_decay', type=float, default=2e-06)
