...
class SplitBreastModel(nn.Module):
    """ Best-model for ca detection in both screening and Bx subpop
    ...
    Each generates L and R benign and malignant probs (after softmax)
...
class ImageBreastModel(nn.Module):
    """ 'image-wise' in paper Fib 5.b
    Seperate fc for each image (per view per breast) """
    def __init__(self, input_channels):
