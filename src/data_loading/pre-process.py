...
from src.data_loading.loading import load_image
from src.data_loading.loading import standard_normalize_single_image
from src.data_loading.augmentations import simple_resize, sample_crop_best_center
...
def crop_mammogram(input_data_folder, input_data_format, exam_list_path, cropped_exam_list_path, output_data_folder,
...
#    print('\nexam_list:', exam_list)
#    input()
    image_list = data_handling.unpack_exam_into_images(
            exam_list,
            cropped=True, 
            centred=True
            )
...
#    input()
    ...
    if os.path.exists(output_data_folder):
        ...
        print("Error: the directory {} already exists.".format(output_data_folder))
        return
    else:
...
def crop_mammogram_one_image(scan, input_file_path, input_file_format, output_file_path, num_iterations, buffer_size):
    """
    Crops a mammogram, saves as png file, includes the following additional information:
        ...
        - distance_from_starting_side: number of zero columns between the start of the image and start of
           the largest connected component w.r.t. original dicom image.
    """
    assert isinstance(input_file_format, str)
    view_input_size = INPUT_SIZE_DICT[scan['full_view']]
    random_number_generator = np.random.RandomState(0)
    
    image = load_image(
            image_path=input_file_path,
            view=scan['view'],
            horizontal_flip=scan['horizontal_flip']
            )
    # plt.imshow(image[:,:,0])
    # plt.title(image.shape)
    # plt.show()
    print('\nDownscaling based on height preserving aspect ratio...\n')
    resized_image = simple_resize(
            image,
            new_height=view_input_size[0]
            )
    
    # if width still too big, scale down again by width:
    if resized_image.shape[1] > view_input_size[1]:
        print('width still too big, downscaling again by width...\n')
        resized_image = resized_image.T # to use 'height' in resize
        resized_image = simple_resize(
            resized_image,
            new_height=view_input_size[1]
            )
        resized_image = resized_image.T # rotate to upright
    # plt.imshow(resized_image)
    # plt.title(resized_image.shape)
    # plt.show()
    
    print('Expanding dims')
    joint_image = np.expand_dims(resized_image, 2)
    
    if resized_image.shape != view_input_size:
        if resized_image.shape[1] < view_input_size[1]:
            print('\nCropped image is too small\n')
        elif resized_image.shape[1] > view_input_size[1]:
            print('{} Width too big'.format(input_file_path))
#            raise ValueError
        elif resized_image.shape[0] < view_input_size[0]:
            print('{} too short'.format(input_file_path))
        print('sample_crop_best_center()...')
        joint_image, borders = sample_crop_best_center(
                image=joint_image, 
                input_size=view_input_size, 
                random_number_generator=random_number_generator, 
                max_crop_noise=(0,0),
                max_crop_size_noise=0, 
                best_center=scan['best_center'], 
                view=scan['full_view'])  
        
#        plt.figure()
#        plt.imshow(image) #joint_image[:,:,0])
#        plt.scatter(3434, 2407, s=40, c='r')
##        plt.imshow(resized_image)
#        plt.show()
##        plt.imshow(joint_image[:,:,0])
##        plt.title(joint_image.shape)
##        plt.show()

    joint_image = standard_normalize_single_image(joint_image)
...
#        print(joint_image)
        ...
        # need to investigation cropping info downstream *
    return ['cropping_info (see crop_mammogram_one_image())']


def crop_mammogram_one_image_short_path(scan, input_data_folder, input_data_format, output_data_folder,
    ...
    """
    full_input_file_path = os.path.join(input_data_folder, scan['short_file_path']+'.hdf5')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--input-data-format', required=True)
...
#    input_file_path = '/data/james/BSSA_dcm_files/ca_free_pts/ML_01_files/cropped/I16206-E13-A913046-S-i1_R_CC_p1i1.hdf5'
#    input_file_format = 'hdf5'
#    input_data_folder = '/data/james/BSSA_dcm_files/ca_free_pts/ML_01_files/cropped'
#    exam_list_path = '/data/james/BSSA_dcm_files/ca_free_pts/ML_01_files/ca_free_pts_ML_01_files_exam_list_3_opt_cent.pkl'
#    cropped_exam_list_path = '/data/james/BSSA_dcm_files/ca_free_pts/ML_01_files/ca_free_pts_ML_01_files_exam_list_4_resized.pkl'
#    output_data_folder = '/data/james/BSSA_dcm_files/ca_free_pts/ML_01_files/resized'
##    
#    crop_mammogram(
#            input_data_folder=input_data_folder, 
...
#            output_data_folder=output_data_folder, 
#            num_processes=args.num_processes,
#            num_iterations=100, #args.num_iterations,
#            buffer_size=50 #args.buffer_size,
#        )