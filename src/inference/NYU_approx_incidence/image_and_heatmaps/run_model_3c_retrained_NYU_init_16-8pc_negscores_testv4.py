...
# ==============================================================================
"""
Runs the image + heatmaps model for breast cancer prediction on BSSA data.
Defaults preset below to run without .sh
Uses 'train_models/ckpts_21Mar_wd2e-06_v1/21Mar_wd2e-06_v1_epo=14_tloss=0.926_avloss=2.035.ckpt'
(best model for 5050 balanced validation set loss)
"""  
import argparse
...
#                import IPython; IPython.embed()
                L_CC_non_malig = batch_predictions['left_malignant', 'CC'][0][0]
                L_MLO_non_malig = batch_predictions['left_malignant', 'MLO'][0][0]
                R_CC_non_malig = batch_predictions['right_malignant', 'CC'][0][0]
                R_MLO_non_malig = batch_predictions['right_malignant', 'MLO'][0][0]
                left_norm = (L_CC_non_malig + L_MLO_non_malig) / 2
                right_norm = (R_CC_non_malig + R_MLO_non_malig) / 2
#                IPython.embed()
                predictions = np.array(
                        np.append(predictions, [left_norm, right_norm]),
                        ndmin=2).astype(np.float32)
#                IPython.embed()
                
#                non_malig = max(left_norm, right_norm)
...
#    non_maligs = [x[3] for x in predictions]
#    import IPython; IPython.embed()
    df = pd.DataFrame(preds, columns=LABELS.LIST + ['left_non_malig', 'right_non_malig'])
    df['BSSA_ID'] = IDs
#    df['non_malig'] = non_maligs
...
def main():
    ...
    parser.add_argument('--data-path',
                        default=os.path.join(DATADIR, 'test_ims_master/NY_16-8pc_incidence/data_sf2.pkl'))
    parser.add_argument('--image-path',
                        default=os.path.join(DATADIR, 'test_ims_master/NY_16-8pc_incidence/cropped_images_sf2'))
    parser.add_argument('--output-path',
                        default=os.path.join(DATADIR,
                                             'test_ims_master/NY_16-8pc_incidence/retrained-3-channel/imageheatmaps_ca_and_norm_predictions_v4.csv'
                                             )
                        )
    ...
    parser.add_argument('--heatmaps-path',
                        default=os.path.join(DATADIR, 'test_ims_master/NY_16-8pc_incidence/heatmaps_sf2'))
    ...
    parameters = {
        ...
        "max_crop_noise": (200, 200),
        "max_crop_size_noise": 200,
